    ...
    <table class="datatables-basic table dataTable no-footer dtr-column" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center">
                    {{ $water_unit_meter_month_previous[$row->id] }}
                </td>
                <td class="text-center d-flex">
                    <span class="badge rounded-pill bg-label-info text-black d-flex" text-capitalized="" style="font-size: unset;">
                        ...
                        <input type="text" name="id_room[]" id="room{{ $row->meters_id }}" class="form-control form-control-sm room{{$key}}" value="{{ intval($row->water_unit) }}" onkeydown="handleInput(event,{{ $row->meters_id }}, this.value,{{ $key }})" oninput="editRoom({{ $row->meters_id }}, this.value)" style="background-color: #d6f7fb;border-color: #00bad1;">
                    </span>
                ...
                <td class="text-center text-danger">
                    {{ $row->water_unit - $water_unit_meter_month_previous[$row->id] }}
                </td>
            </tr>
            @endforeach
        </tbody>
